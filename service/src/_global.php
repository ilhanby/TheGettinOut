...
function response_list($source, $order = '')
{
    $res = array();
...
{
    $res = array(
        "header" => array("status" => true),
        "body" => $data
    );
    echo json_encode($res);
}

function response_error($code, $message, $body = null)
{
    $res = array(
        "header" => array("status" => false, "code" => $code, "message" => $message),
        "body" => $body
    );
    echo json_encode($res);
    die();
